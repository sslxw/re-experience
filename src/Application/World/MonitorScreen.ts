...
import { isCursorMotionLocked } from '../UI/cursorMotionLock';
...
export default class MonitorScreen extends EventEmitter {
    ...
    screenIframe: HTMLIFrameElement | null;
    ...
    isScreenTarget(target: EventTarget | null) {
        const el = target as HTMLElement | null;
        if (!el) return false;
        return (
            el.id === 'computer-screen' ||
            el.id === 'computer-screen-container' ||
            !!el.closest?.('#computer-screen-container')
        );
    }

    activateAtMonitor() {
        if (this.atMonitor) return;
        this.atMonitor = true;
        this.inComputer = true;
        this.prevInComputer = true;
        if (this.screenIframe) {
            this.screenIframe.style.pointerEvents = 'auto';
        }
        if (this.screenContainer) {
            this.screenContainer.style.cursor = 'default';
        }
        this.camera.trigger('enterMonitor');
    }

    setEventInComputer(event: MouseEvent) {
        const onScreen = this.isScreenTarget(event.target);
        // @ts-ignore
        event.inComputer = this.atMonitor ? true : onScreen;
    }
    ...
    initializeScreenEvents() {
        document.addEventListener(
            'mousemove',
            (event) => {
                if (isCursorMotionLocked()) return;
                this.setEventInComputer(event);
                // @ts-ignore
        ...
        document.addEventListener(
            ...
            (event) => {
                ...
                if (onScreen && !this.atMonitor) {
                    event.preventDefault();
                    event.stopPropagation();
                    // @ts-ignore
                    event.inComputer = true;
                    this.activateAtMonitor();
                } else {
                    this.setEventInComputer(event);
                    // @ts-ignore
                    this.inComputer = event.inComputer;
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && this.atMonitor) {
                event.preventDefault();
                this.leaveMonitor();
            }
        });

        UIEventBus.on('requestLeaveMonitor', () => {
            if (this.atMonitor) {
                this.leaveMonitor();
            }
        });
        ...
        document.addEventListener(
            ...
            (event) => {
                if (isCursorMotionLocked()) return;
                if (!this.atMonitor || !this.screenIframe) return;
                if (!this.isScreenTarget(event.target)) return;

                const root =
                    this.screenIframe.contentDocument?.getElementById(
                        'view-root'
                    );
                if (!root) return;
    ...
    leaveMonitor() {
        ...
        this.mouseClickInProgress = false;
        if (this.screenIframe) {
            this.screenIframe.style.pointerEvents = 'none';
        }
        if (this.screenContainer) {
            this.screenContainer.style.cursor = 'pointer';
        }
    ...
    createIframe() {
        ...
        container.style.background = '#080604';
        container.id = 'computer-screen-container';
        ...
        container.addEventListener(
            'mousedown',
            (event) => {
                if (isCursorMotionLocked() || this.atMonitor) return;
                event.preventDefault();
                event.stopPropagation();
                this.activateAtMonitor();
            },
        ...
        iframe.src = '/computer-screen/index.html';
        ...
        iframe.title = 'Raccoon City Archive';
    ...
    update() {
        if (this.dimmingPlane) {
            ...
            const mat = this.dimmingPlane.material as THREE.MeshBasicMaterial;
